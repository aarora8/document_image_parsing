...
# def get_horizontal_angle(rectangle):
#     given_angle = rectangle.unit_vector_angle
#     if given_angle > pi/2:
#         given_angle = given_angle - pi
#     elif given_angle < -pi/2:
#         given_angle = given_angle + pi
#     return given_angle

def get_horizontal_angle(rectangle):
    given_angle = rectangle.unit_vector_angle
    ortho_vector = orthogonal_vector(rectangle.unit_vector)
    new_given_angle = atan2(ortho_vector[1], ortho_vector[0])
    print(degrees(new_given_angle))
    # if given_angle > 0 and given_angle < pi / 2:
    #     given_angle = given_angle
    if given_angle > pi / 2 and given_angle < pi:
        given_angle = given_angle - pi
    # elif given_angle > pi and given_angle < 3 * pi / 2:
    #     given_angle = given_angle - 3 * pi / 2
    # elif given_angle > 3 * pi / 2 and given_angle < 2 * pi:
    #     given_angle = given_angle - 2 * pi
    # elif given_angle > -pi / 2 and given_angle < 0:
    #     given_angle = given_angle + pi / 2
    elif given_angle > -pi and given_angle < -pi / 2:
        given_angle = given_angle + pi
    # elif given_angle > -pi and given_angle < -3 * pi / 2:
    #     given_angle = given_angle + 3 * pi / 2
    # elif given_angle > -3 * pi / 2 and given_angle < -2 * pi:
...
def rotated_points(bounding_box, center):
    ...
    rotation_angle_in_rad = -get_horizontal_angle(bounding_box)
    print(degrees(-bounding_box.unit_vector_angle))
    print(degrees(-get_horizontal_angle(bounding_box)))
