    ...
    for node in zone:
        ...
        if id == 'z13':
            ...
            for line_node in line_point:
                col_line.append(int(line_node.getAttribute('x')))
                row_line.append(int(line_node.getAttribute('y')))

            max_col, max_row = max(col_line), max(row_line)
            min_col, min_row = min(col_line), min(row_line)
            ...
            plt.fill((x_sort[2], x_sort[1], x_sort[0], x_sort[3]), (y_sort[0], y_sort[1], y_sort[3], y_sort[2]),
                     alpha=0.5)
            plt.axis('equal')
